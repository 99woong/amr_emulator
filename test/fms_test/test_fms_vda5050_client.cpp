...
#include <mqtt/client.h> // Eclipse Paho MQTT C++ client
#include <json.hpp> // VDA 5050 JSON 메시지 생성을 위한 라이브러리
...
//     nlohmann::json node_path;
//     // 경로 예시: 시작 노드 -> 목표 노드
//     node_path.push_back({{"nodeId", startNode}, {"sequenceId", 0}});
//     node_path.push_back({{"nodeId", targetNode}, {"sequenceId", 1}});
...
//         int current_order_id = 1;
//         std::string target_agv_id_prefix = "amr_"; // AMR ID 접두사
//         int amr_count = 1; // 테스트할 AMR 개수 (AmrManager config와 일치시켜야 함)
